...
func RequestPost(w http.ResponseWriter, r *http.Request) {
	...
	query := `SELECT id, username, postcontent, timestamp, imagepath FROM POSTS ORDER BY id DESC`
	...
	for rows.Next() {
		var post PostData
		err := rows.Scan(&post.Id, &post.Username, &post.PostContent, &post.Timestamp, &post.Imagepath)
		if err != nil {
			log.Fatal(err)
		}
...
func RequestComment(w http.ResponseWriter, r *http.Request) {
	...
	query := `SELECT id, username, postcontent, timestamp FROM COMMENTS WHERE parentpostid = ?`
	...
	for rows.Next() {
		...
		err := rows.Scan(&comment.Id, &comment.Username, &comment.PostContent, &comment.Timestamp)
		if err != nil {
			log.Fatal(err)
		}
